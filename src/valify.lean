...
| `(tactic| valify $[[$simpArgs,*]]? $[at $location]?) =>
  ...
  `(tactic|
    simp -decide only [ZMod.val_sub, ZMod.val_add, ZMod.val_mul, ZMod.val_one, push_cast, norm_num, $args,*] $[at $location]? )
...
def mkZifyContext (simpArgs : Option (Syntax.TSepArray `Lean.Parser.Tactic.simpStar ",")) :
  ...
  let args := simpArgs.map (·.getElems) |>.getD #[]
  mkSimpContext
    (← `(tactic| simp; simp  [ZMod.val_sub, ZMod.val_add, ZMod.val_mul, ZMod.val_one, push_cast, norm_num, $args,*] ; apply [Nat.lt_of_le_of_lt] )) false
...
example (x: ZMod 4139) (h: ZMod.val x ≤ ZMod.val (1 : ZMod ff)):
  (1- x).val = (ZMod.val (1 : ZMod ff) - x.val) := by
  valify at h
  valify [h]
