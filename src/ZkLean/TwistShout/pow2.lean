...
def VirtualPow2_32 [Field f] : Subtable f 64 :=
  subtableFromMLE (fun x => 1*(1 + 1*x[63])*(1 + 3*x[62])*(1 + 15*x[61])*(1 + 255*x[60])*(1 + 65535*x[59]))

  --subtableFromMLE (fun x => 1*(1 + 1*bv2[0])*(1 + 3*bv2[1])*(1 + 15*bv2[2])*(1 + 255*bv2[3])*(1 + 65535*bv2[4]))





lemma move_sign_32_mle_one_chunk_[ZKField f] (bv1 bv2 : BitVec 32) (fv1 fv2 : Vector f 32) :
...
((1#32) + (((bv2 >>> 4) &&& 1#32) * (65535#32)))) =
  (foutput = evalSubtable VirtualPow2_32 (Vector.append fv1 fv2))) := by
    solveMLE VirtualPow2_32 32
